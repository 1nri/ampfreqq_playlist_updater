...
public class APEGUI extends JFrame {
    ...
    public APEGUI() {
        ...
        previewButton = new JButton("View Playlist");
...
/*
    ...
    private void previewChanges() {
        ...
        if (modifiedLines != null) {
            ...
            logChange("Preview: showing unsaved runtime content.");
            return;
        }
        ...
        try {
            ...
            outputArea.setText(String.join("\n", lines));
            logChange("Preview: showing file content from disk.");
    ...
    private void insertLine() {
        ...
        if (newContent.isEmpty()) {
            outputArea.setText("Please provide content for the new row.");
            return;
        }
        ...
        try {
            ...
            outputArea.setText("Invalid row number.");
            return;
        }

        try {
            List<String> workingLines = (modifiedLines != null)
                    ? new ArrayList<>(modifiedLines)
                    : Files.readAllLines(Paths.get(filePath));

            List<String> contentList = new ArrayList<>();
            for (String line : workingLines) {
                int commaIndex = line.indexOf(',');
                if (commaIndex != -1) {
                    contentList.add(line.substring(commaIndex + 1).trim());
                } else {
            ...
            for (int i = 0; i < contentList.size(); i++) {
                runtimeLines.add(i + ", " + contentList.get(i));
            }

            modifiedLines = runtimeLines;
            outputArea.setText(String.join("\n", runtimeLines));
            logChange("Row added at " + insertAt + " (runtime). Use Save Changes to write to disk.");
        } catch (IOException e) {
            outputArea.setText("Failed to update runtime content: " + e.getMessage());
        }
    }
